...
##LRT all results
ordered_res_group_table <- fread("output/deseq2/tissue_LRT/all_res.csv")
...
vd_head <- venn.diagram(x = list("itWT head DEGs"=head_specific_DEGs$V1, "LRT"=ordered_sig_res_group_table$rn), filename=NULL, alpha=0.7, cex = 1, cat.cex=1, lwd=1.5)
grid.newpage()
grid.draw(vd_head)
...
##not sig in lrt?
not_LRT_head_degs_ids <- setdiff(head_specific_DEGs$V1, ordered_sig_res_group_table$rn)
...
vd_thor <- venn.diagram(x = list("itWT thorax DEGs"=thorax_specific_DEGs$V1, "LRT"=ordered_sig_res_group_table$rn), filename=NULL, alpha=0.7, cex = 1, cat.cex=1, lwd=1.5)
grid.newpage()
grid.draw(vd_thor)
...
##not sig in lrt?
not_LRT_thorax_degs_ids <- setdiff(thorax_specific_DEGs$V1, ordered_sig_res_group_table$rn)
...
vd_abdo <- venn.diagram(x = list("itWT abdo DEGs"=abdo_specific_DEGs$V1, "LRT"=ordered_sig_res_group_table$rn), filename=NULL, alpha=0.7, cex = 1, cat.cex=1, lwd=1.5)
grid.newpage()
grid.draw(vd_abdo)
