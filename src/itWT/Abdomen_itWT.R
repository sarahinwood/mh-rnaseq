...
##abdo head
abdo_head <- results(mh_dds, contrast=c("Tissue", "Abdomen", "Head"), alpha = 0.05, lfcThreshold = 1)
summary(abdo_head)
##Order based of padj
...
##abdo thorax
abdo_thorax <- results(mh_dds, contrast=c("Tissue", "Abdomen", "Thorax"), alpha = 0.05, lfcThreshold = 1)
summary(abdo_thorax)
##Order based of padj
...
##abdo thorax
abdo_venom <- results(mh_dds, contrast=c("Tissue", "Abdomen", "Venom"), alpha = 0.05, lfcThreshold = 1)
summary(abdo_venom)
##Order based of padj
...
##abdo ovaries 
abdo_ovaries <- results(mh_dds, contrast=c("Tissue", "Abdomen", "Ovaries"), alpha = 0.05, lfcThreshold = 1)
...
fwrite(list(abdo_specific_DEGs), "output/deseq2/itWT/abdo_itWT/mh_abdo_specific_degs.txt")

abdo_sp_annots <- subset(trinotate, `#gene_id` %in% abdo_specific_DEGs)
fwrite(abdo_sp_annots, "output/deseq2/itWT/abdo_itWT/abdo_sp_annots.csv")
