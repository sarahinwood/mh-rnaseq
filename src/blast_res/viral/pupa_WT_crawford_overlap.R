...
pupa_degs <- fread("output/deseq2/stage_WT/sig_annots.csv")
...
vd1 <- venn.diagram(x = list("Pupa DEGs"=pupa_degs$rn, "Prodigal Viral"=prodigal_viral_genes_blast$Trinity_ID,
                             "Crawford venom"=crawford_venom_blast$Trinity_ID), filename=NULL, alpha=0.7, cex = 1, cat.cex=1, lwd=1.5)
grid.newpage()
grid.draw(vd1)

pupa_viral <- intersect(pupa_degs$rn, prodigal_viral_genes_blast$Trinity_ID)
pupa_venom <- intersect(pupa_degs$rn, crawford_venom_blast$Trinity_ID)
...
annot_counts <- filter(counts_table, rn %in% pupa_viral)
